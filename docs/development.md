...
Search MCP Serverは、**MCPアグリゲーター**として、複数のMCPサーバーを統合管理します。直接的なツール実装よりも、MCPプロトコルの正確な実装とプロキシ機能に注力します。
...
**原則**: 新しいMCPサーバーの追加が容易な設計

- 設定ファイルベースの管理
- 既存のMCPサーバーをそのまま利用可能
- バックエンドMCPサーバー間の独立性を保つ
